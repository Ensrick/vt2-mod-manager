...
/// <summary>
/// Wraps <c>ISteamRemoteStorage/GetPublishedFileDetails/v1/</c>. No API key required for public
/// items; private/friends-only items return result=9 with no fields, surfaced as a
/// <see cref="WorkshopItemRemote"/> with ApiResult=9 and every other field null.
///
...
public sealed class SteamWebApiClient
{
    ...
    public static Dictionary<ulong, WorkshopItemRemote> ParseResponse(string json, IEnumerable<ulong> requested)
    {
        ...
        catch (JsonException) { return MarkAllFailed(requested); }
