...
// Ported VERBATIM from workshop-sentinel @ commit a097dfa
// (Services/SteamCefCookieReader.cs). Only the namespace and a cosmetic temp-file
// prefix have been changed. WS is WIP; treat this whole file as best-effort.
...
public sealed class SteamCefCookieReader
{
    ...
    public SteamCefCookieReadResult Read()
    {
        ...
        var tempDb = Path.Combine(Path.GetTempPath(), $"vt2-mod-manager-cookies-{Guid.NewGuid():N}.db");
        try
        {
