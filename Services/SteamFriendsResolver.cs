...
/// <summary>
/// A friend entry pulled out of Steam's local userdata. Mirrors the WS shape so the call
/// site can map it onto vt2-mod-manager's persisted <see cref="Friend"/> record.
/// </summary>
public sealed record SteamFriendListing
{
...
/// modern Steam — they live in the Friends UI's Chromium IndexedDB). vt2-mod-manager
/// persists its own favorites flag on <see cref="Friend"/>.
/// </summary>
public sealed class SteamFriendsResolver
{
    ...
    /// </summary>
    public IReadOnlyList<SteamFriendListing> Resolve()
    {
        var userdata = Path.Combine(_steamRoot, "userdata");
        if (!Directory.Exists(userdata)) return Array.Empty<SteamFriendListing>();
        ...
        var combined = new Dictionary<string, SteamFriendListing>();   // dedupe by SteamID64 across accounts
    ...
    /// </summary>
    public static IEnumerable<SteamFriendListing> ParseLocalConfig(string path, uint selfAccountId)
    {
        AcfNode root;
        ...
        foreach (var (key, body) in friendsNode.Children)
        {
            ...
            yield return new SteamFriendListing
            {
