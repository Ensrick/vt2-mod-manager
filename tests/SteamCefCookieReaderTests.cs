...
// Ported from workshop-sentinel tests/SteamCefCookieReaderTests.cs.
// Namespace and temp-file prefix changed; assertions are unchanged.
...
public class SteamCefCookieReaderTests
{
    ...
    {
        var tmp = Path.Combine(Path.GetTempPath(), $"vt2-mod-manager-localstate-{Guid.NewGuid():N}.json");
    ...
    {
        var nonexistent = Path.Combine(Path.GetTempPath(), $"vt2-mod-manager-nohtmlcache-{Guid.NewGuid():N}");
